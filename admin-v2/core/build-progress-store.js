(() => {
  ...
  const workItems=Object.freeze([
    ...
    {id:'legacy-home',group:'긴급 개편',title:'어드민 메인과 본부 화면 완전 분리',status:'verified',route:'command-home'},
    {id:'legacy-layout',group:'긴급 개편',title:'카드 과밀을 탐색기·표·목록 구조로 정리',status:'verified',route:'tool-build-progress'},
  ...
  const defaults=Object.freeze({schemaVersion:SCHEMA_VERSION,currentPhase:'Admin Workspace 실사용화',status:'in_progress',percent:'86',currentTask:'각 카테고리 Workspace에 실제 데이터 표와 실행 기능 연결',completed:'사용자 동영상으로 기존 실패 상태 확인\n카테고리 전용 Workspace 컨테이너 생성\n메인과 본부 화면 완전 분리\n동적 리나 메인 재생성 시에도 Workspace만 유지\n왼쪽 대분류·소분류 독립 화면 전환\n탐색기형 하위 업무 목록 적용\n카드 과밀을 표·목록 중심으로 축소\nURL·활성 메뉴·상단 제목·뒤로가기 연결',remaining:'각 카테고리별 실제 데이터와 동작 연결\n왼쪽 전체 카테고리 브라우저 클릭 전수 확인\n모바일 전환 검증\n실제 Google API Bridge 연결\n최종 Runtime·Production E2E',note:'전용 Workspace 구조를 GitHub main에 반영했다. 운영 브라우저 전수 클릭 전에는 최종 완료로 판정하지 않는다.',workItems,history:[],runtimeAudit:null});
