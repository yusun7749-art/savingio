(() => {
  ...
  const HISTORY_KEY = 'savingio-lina-health-history';
  const JOURNAL_KEY = 'savingio-lina-voyage-journal';
  const RETIRE_KEY = 'savingio-lina-last-retire';
  ...
  const state = {
    ...
    health: 0,
    doctor: { rows: 0, duplicate: 0, failed: 0, approved: 0, projects: 0, lastAudit: '' }
  };
  ...
  const taskKey = id => `savingio-lina-task:${todayKey()}:${id}`;
  ...
  function readCounts() {
    ...
    const duplicate = rows.filter(row => {
      const risk = numberFromText(row.querySelector('.duplicate-risk')?.textContent);
      return risk >= 50;
    }).length;
    const failed = rows.filter(row => {
      const detail = row.querySelector('.audit-detail-btn')?.textContent || '';
      const grade = row.querySelector('.quality-grade')?.textContent?.trim() || '';
      return /❌|미달/.test(detail) || ['B', 'C', 'D'].includes(grade);
    }).length;
    const approved = rows.filter(row => {
      const status = row.querySelector('.content-status')?.textContent || '';
    ...
    const lastAudit = $('#contentLastAudit')?.textContent?.trim() || '';
    state.doctor = { rows: rows.length, duplicate, failed, approved, projects, lastAudit };
    return state.doctor;
  }
  ...
  function calculateHealth(counts) {
    ...
    const qualityPenalty = counts.rows ? Math.min(24, Math.round((counts.failed / total) * 24)) : 8;
    const duplicatePenalty = counts.rows ? Math.min(16, Math.round((counts.duplicate / total) * 16)) : 4;
  ...
  function priorityTasks(counts) {
    ...
    if (counts.rows === 0) tasks.push({ id: 'doctor', score: 86, gain: 8, level: '중요', title: '전체 Doctor 검사 실행', note: '현재 Doctor 결과가 없습니다.', reason: '콘텐츠 품질과 중복 상태를 읽어야 정확한 우선순위를 계산할 수 있습니다.', target: '#contentApprovalCenter' });
    if (counts.failed > 0) tasks.push({ id: 'quality', score: 82, gain: 5, level: '중요', title: '헌법/DNA 미달 글 확인', note: `현재 미달 ${counts.failed}건`, reason: '품질 미달 콘텐츠는 애드센스 승인과 검색 신뢰도에 영향을 줍니다.', target: '#contentApprovalCenter' });
    if (counts.duplicate > 0) tasks.push({ id: 'duplicate', score: 76, gain: 4, level: '중요', title: '중복 콘텐츠 검토', note: `중복 위험 50% 이상 ${counts.duplicate}건`, reason: '검색 의도가 겹치는 글은 서로의 노출 가능성을 낮출 수 있습니다.', target: '#contentApprovalCenter' });
  ...
  function savedDone(id) {
    return localStorage.getItem(taskKey(id)) === 'done';
  }

  function setDone(id, done) {
    done ? localStorage.setItem(taskKey(id), 'done') : localStorage.removeItem(taskKey(id));
  }
  ...
  function recordHealth() {
    const snapshot = {
      date: todayKey(), health: state.health, projected: projectedHealth(),
      failed: state.doctor.failed, duplicate: state.doctor.duplicate,
      broken: integrityCounts().broken, recordedAt: new Date().toISOString()
    };
    const history = readHealthHistory().filter(item => item.date !== snapshot.date);
    history.push(snapshot);
    localStorage.setItem(HISTORY_KEY, JSON.stringify(history.slice(-30)));
  }

  function renderHealthHistory() {
    const box = $('#linaHealthHistory');
    if (!box) return;
    const live = { date: todayKey(), health: state.health, projected: projectedHealth() };
    const merged = [...readHealthHistory().filter(item => item.date !== live.date), live].slice(-7);
    box.innerHTML = merged.map(item => `
      <div class="world-row">
        <div class="world-label"><strong>${esc(item.date)}</strong><span>현재 ${Number(item.health || 0)} · 예상 ${Number(item.projected || item.health || 0)}</span></div>
        <div class="world-track"><div class="world-fill" style="width:${Math.max(0, Math.min(100, Number(item.health || 0)))}%"></div></div>
        <b>${Number(item.health || 0)}</b>
      </div>`).join('') || '<p class="lina-home-message">Health 기록이 아직 없습니다.</p>';
  }
  ...
  function renderTasks() {
    ...
    box.querySelectorAll('[data-lina-task]').forEach(input => input.addEventListener('change', () => {
      setDone(input.dataset.linaTask, input.checked);
      updateProgress();
      renderDecisionPanel();
      renderJournal();
    }));
  ...
  function greeting() {
    ...
    if (hour < 11) return '좋은 아침입니다, 선장님.';
    if (hour < 18) return '선장님, 오늘 작업 이어가겠습니다.';
    return '선장님, 오늘 작업을 정리할 시간입니다.';
  }
  ...
  function renderWorldMap(counts) {
    ...
    const items = [
      ...
      ['품질', counts.rows ? percent(counts.failed, total, true) : 0, counts.rows ? `${counts.failed}개 미달` : 'Doctor 검사 필요'],
      ['중복', counts.rows ? percent(counts.duplicate, total, true) : 0, counts.rows ? `${counts.duplicate}건 위험` : 'Doctor 검사 필요'],
    ...
    if (box) box.innerHTML = items.map(([name, value, note]) => `<div class="world-row"><div class="world-label"><strong>${name}</strong><span>${note}</span></div><div class="world-track"><div class="world-fill" style="width:${value}%"></div></div><b>${value}%</b></div>`).join('');
  }
  ...
  function writeJournal(entry) {
    ...
    localStorage.setItem(JOURNAL_KEY, JSON.stringify(journal.slice(0, 14)));
  }
  ...
  function renderJournal() {
    ...
    box.innerHTML = journal.map(item => `<article class="voyage-entry ${item.live ? 'current' : ''}"><div><strong>${esc(item.date)}</strong><span>${item.live ? '오늘 항해 중' : '항해 기록'}</span></div><ul>${item.completed?.length ? item.completed.map(text => `<li>완료 · ${esc(text)}</li>`).join('') : '<li>진행 기록 없음</li>'}</ul><p>다음: ${esc(item.next || '미정')}</p></article>`).join('');
  }
  ...
  function renderDecisionPanel() {
    ...
    box.innerHTML = active.length ? active.map((task, index) => `
      <article class="lina-decision-item">
        <div class="world-row"><div class="world-label"><strong>${index + 1}. [${esc(task.level)}] ${esc(task.title)}</strong><span>${esc(task.reason)}</span><small>${esc(task.note)}</small></div><b>+${task.gain}</b></div>
        <div class="lina-command"><button class="btn ghost small" type="button" data-decision-target="${esc(task.target)}">대상 열기</button><button class="btn primary small" type="button" data-decision-done="${esc(task.id)}">오늘 처리 완료</button></div>
      </article>`).join('') : '<p class="lina-home-message pass">오늘 우선 작업이 모두 완료되었습니다.</p>';
    box.querySelectorAll('[data-decision-target]').forEach(button => button.addEventListener('click', () => document.querySelector(button.dataset.decisionTarget)?.scrollIntoView({ behavior: 'smooth', block: 'start' })));
    box.querySelectorAll('[data-decision-done]').forEach(button => button.addEventListener('click', () => {
      setDone(button.dataset.decisionDone, true);
      renderDashboard();
    }));
    if ($('#linaProjectedHealth')) $('#linaProjectedHealth').textContent = `${projectedHealth()}/100`;
  }
  ...
  function renderIntegrityCard() {
    ...
    if ($('#linaIntegrityStatus')) { $('#linaIntegrityStatus').textContent = state.integrityLoaded ? integrity.status : 'LOADING'; $('#linaIntegrityStatus').className = statusClass; }
    ...
    if ($('#linaDoctorStatus')) $('#linaDoctorStatus').textContent = state.doctor.rows > 0 ? `${state.doctor.rows}개 분석` : '검사 필요';
    if ($('#linaDoctorAuditTime')) $('#linaDoctorAuditTime').textContent = state.doctor.lastAudit || '기록 없음';
  }
  ...
  function renderDashboard() {
    ...
    if ($('#linaGreetingTitle')) $('#linaGreetingTitle').textContent = greeting();
    if ($('#linaGreetingText')) $('#linaGreetingText').textContent = first ? `운영 건강도 ${state.health}점입니다. 오늘 최우선 작업은 “${first.title}”이며, 우선순위 ${first.score}점입니다.` : '오늘은 전체 Doctor 검사로 현재 콘텐츠 상태부터 확인해야 합니다.';
    if ($('#linaBrainDuplicate')) $('#linaBrainDuplicate').textContent = counts.duplicate;
    if ($('#linaBrainWeak')) $('#linaBrainWeak').textContent = counts.failed;
    if ($('#linaBrainPublished')) $('#linaBrainPublished').textContent = counts.rows;
    if ($('#linaBrainProjects')) $('#linaBrainProjects').textContent = counts.projects;
  ...
  function mount() {
    ...
    $('.stats').insertAdjacentHTML('afterend', `
      <section class="lina-home" id="linaHome">
        <div class="lina-welcome">
          <article class="lina-greeting"><p class="eyebrow">Lina HQ · 실제 운영 브리핑</p><h2 id="linaGreetingTitle">선장님, 리나가 실제 운영 데이터를 확인하고 있습니다.</h2><p id="linaGreetingText">콘텐츠·중복·사이트 무결성 상태를 읽는 중입니다.</p><div class="lina-status-row"><span class="lina-status-pill ok">GitHub main 연결</span><span class="lina-status-pill ok">신뢰 기기</span><span class="lina-status-pill warn">운영 URL 검증 필요</span></div></article>
        ...
        <div class="lina-brain-grid"><article class="lina-brain-card" data-lina-scroll="#contentApprovalCenter"><span>중복 위험</span><strong id="linaBrainDuplicate">0</strong><small>위험도 50% 이상</small></article><article class="lina-brain-card" data-lina-scroll="#contentApprovalCenter"><span>품질 미달</span><strong id="linaBrainWeak">0</strong><small>B·C·D 또는 미달 항목</small></article><article class="lina-brain-card" data-lina-scroll="#contentApprovalCenter"><span>확인된 콘텐츠</span><strong id="linaBrainPublished">0</strong><small>Doctor 검사에서 읽은 글</small></article><article class="lina-brain-card" data-lina-scroll=".workspace-grid"><span>운영 프로젝트</span><strong id="linaBrainProjects">0</strong><small>현재 프로젝트 작업판</small></article></div>
        <article class="lina-integrity-card panel"><div class="lina-card-head"><div><p class="eyebrow">Real Operations Data</p><h3>사이트 무결성·Health 보고서</h3></div><strong id="linaIntegrityStatus">LOADING</strong></div><div class="captain-metrics"><span>Health <strong id="linaHealthScore">0/100</strong></span><span>상위 3개 처리 예상 <strong id="linaProjectedHealth">0/100</strong></span><span>판정 <strong id="linaHealthStatus">계산 중</strong></span><span>Doctor <strong id="linaDoctorStatus">연결 중</strong></span><span>최근 검사 <strong id="linaDoctorAuditTime">-</strong></span><span>검사 파일 <strong id="linaIntegrityFiles">0</strong></span><span>깨진 참조 <strong id="linaIntegrityBroken">0</strong></span><span>사이트맵 누락 <strong id="linaIntegritySitemap">0</strong></span><span>Publisher LOCK <strong id="linaPublisherLock">-</strong></span></div></article>
        <article class="lina-world-card panel"><div class="lina-card-head"><div><p class="eyebrow">Lina Decision Engine</p><h3>우선순위 판단과 실행 패널</h3></div><span class="map-live">AUTO</span></div><div id="linaDecisionList" class="world-map"></div></article>
        <article class="lina-world-card panel"><div class="lina-card-head"><div><p class="eyebrow">Health History</p><h3>최근 7일 운영 건강도</h3></div><span class="map-live">LOCAL</span></div><div id="linaHealthHistory" class="world-map"></div></article>
    ...
    const start = () => {
      ...
      if ($('#linaHomeMessage')) { $('#linaHomeMessage').textContent = first ? `선장님, ${first.title}부터 시작합니다. 판단 이유: ${first.reason}` : '오늘 필수 작업이 모두 완료되었습니다.'; $('#linaHomeMessage').className = 'lina-home-message pass'; }
    };
    ...
    $('#linaRetireBtn')?.addEventListener('click', () => {
      ...
      localStorage.setItem(RETIRE_KEY, JSON.stringify({ date: new Date().toISOString(), completed: done, next, health: state.health, projectedHealth: projectedHealth() }));
    ...
    const observer = new MutationObserver(() => {
      ...
      observer.timer = setTimeout(renderDashboard, 180);
    });
    if ($('#contentApprovalRows')) observer.observe($('#contentApprovalRows'), { childList: true, subtree: true });
    if ($('#contentLastAudit')) observer.observe($('#contentLastAudit'), { childList: true, subtree: true, characterData: true });
