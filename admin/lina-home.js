(() => {
  ...
  function recordHealth() {
    const snapshot = {
      ...
      failed: state.doctor.failed,
      duplicate: state.doctor.duplicate,
      broken: integrityCounts().broken,
  ...
  function renderHealthHistory() {
    ...
    if (!box) return;
    const history = readHealthHistory().slice(-7);
    const live = { date: todayKey(), health: state.health, projected: projectedHealth() };
    const merged = [...history.filter(item => item.date !== live.date), live].slice(-7);
    box.innerHTML = merged.length ? merged.map(item => `
      <div class="world-row">
  ...
  function renderDecisionPanel() {
    ...
    box.innerHTML = active.length ? active.map((task, index) => `
      <article class="lina-decision-item">
        <div class="world-row">
          <div class="world-label"><strong>${index + 1}. [${esc(task.level)}] ${esc(task.title)}</strong><span>${esc(task.reason)}</span><small>${esc(task.note)}</small></div>
          <b>+${task.gain}</b>
        </div>
        <div class="lina-command"><button class="btn ghost small" type="button" data-decision-target="${esc(task.target)}">대상 열기</button><button class="btn primary small" type="button" data-decision-done="${esc(task.id)}">완료 처리</button></div>
    ...
    box.querySelectorAll('[data-decision-done]').forEach(button => button.addEventListener('click', () => {
      localStorage.setItem(`savingio-lina-task:${todayKey()}:${button.dataset.decisionDone}`, 'done');
      renderDashboard();
    }));
    if ($('#linaProjectedHealth')) $('#linaProjectedHealth').textContent = `${projectedHealth()}/100`;
  }
  ...
  function renderIntegrityCard() {
    ...
    if ($('#linaIntegrityStatus')) { $('#linaIntegrityStatus').textContent = statusText; $('#linaIntegrityStatus').className = statusClass; }
  ...
  function mount() {
    ...
    document.addEventListener('savingio:content-audit-complete', () => {
      renderDashboard();
      recordHealth();
      renderHealthHistory();
    });
