(() => {
  ...
  function similarity(a,b){
    ...
    const sa=new Set((a.path.split('/').pop()||'').replace(/-20\d\d$/,'').split('-').filter(x=>x.length>2));
    const sb=new Set((b.path.split('/').pop()||'').replace(/-20\d\d$/,'').split('-').filter(x=>x.length>2));
  ...
  function showGroup(id){
    ...
    $('#duplicateGroupDetail').innerHTML=`<div class="duplicate-recommendation"><strong>대표 유지 추천</strong><a href="${esc(g.representative.path)}" target="_blank" rel="noopener">${esc(g.representative.title)}</a><p>품질 ${g.representative.score}점 · 본문 ${g.representative.length.toLocaleString()}자 기준으로 그룹 내 가장 강한 글입니다.</p></div><div class="duplicate-comparison-table"><table><thead><tr><th>추천</th><th>글</th><th>대표와 유사도</th><th>품질</th><th>본문</th><th>운영 판단</th></tr></thead><tbody>${g.items.map(i=>`<tr><td><span class="dup-action ${i.action==='대표 유지'?'keep':i.action==='통합 후보'?'merge':'delete'}">${i.action}</span></td><td><a href="${esc(i.path)}" target="_blank" rel="noopener">${esc(i.title)}</a><small>${esc(i.path)}</small></td><td>${i.match}%</td><td>${i.score}점</td><td>${i.length.toLocaleString()}자</td><td><select data-decision-path="${esc(i.path)}"><option ${i.action==='대표 유지'?'selected':''}>대표 유지</option><option ${i.action==='통합 후보'?'selected':''}>통합 후보</option><option ${i.action==='삭제 후보'?'selected':''}>삭제 후보</option><option>판단 보류</option></select></td></tr>`).join('')}</tbody></table></div><div class="duplicate-impact-note"><strong>삭제 전 필수 확인</strong><span>백업 → 내부링크 점검 → 대표 URL 연결 → 사이트맵 제거 → 배포 후 URL 검증</span></div><div class="dialog-actions"><button class="btn ghost" id="duplicateHoldBtn" type="button">판단 보류</button><button class="btn primary" id="duplicateReviewDoneBtn" type="button">이 그룹 검토 완료</button></div>`;
