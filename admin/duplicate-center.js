(() => {
  ...
  function readArticles(){
    return [...document.querySelectorAll('#contentApprovalRows tr[data-path]')].map(row=>{
      ...
      return {
        ...
        score:Number((cells[0]?.querySelector('strong')?.textContent||'0').replace(/\D/g,'')),
        grade:(cells[0]?.querySelector('.quality-grade')?.textContent||'').trim(),
        missingCount:Number((cells[3]?.textContent||'0').replace(/\D/g,'')),
        length:Number((cells[5]?.textContent||'0').replace(/\D/g,''))
      };
    }).filter(x=>x.title&&x.path);
  }

  function contentIdeas(group){
    const repTokens=tokens(group.representative.title), ideas=[];
    group.items.filter(i=>i.path!==group.representative.path).forEach(item=>{
      [...tokens(item.title)].filter(token=>!repTokens.has(token)).forEach(token=>ideas.push(token));
    });
    return [...new Set(ideas)].slice(0,8);
  }

  function qualityPlan(group){
    const rep=group.representative;
    const gaps=[];
    if(rep.length<5000) gaps.push(`본문 ${Math.max(0,5000-rep.length).toLocaleString()}자 이상 보강`);
    if(rep.score<90) gaps.push(`헌법/DNA 미달 ${rep.missingCount||Math.max(1,Math.ceil((90-rep.score)/6))}개 해결`);
    if(rep.score<94) gaps.push('FAQ·비교표·체크리스트 재점검');
    if(group.items.length>1) gaps.push(`중복 ${group.items.length-1}개 핵심 정보 흡수`);
    gaps.push('내부링크·공식기관·계산기 연결 확인');
    const ideas=contentIdeas(group);
    const projectedLength=Math.max(5200, rep.length + group.items.filter(i=>i.path!==rep.path).reduce((sum,i)=>sum+Math.min(900,Math.round(i.length*.18)),0));
    const projectedScore=Math.min(100, Math.max(94, rep.score + Math.min(12,(group.items.length-1)*3) + (rep.length<5000?4:0)));
    const seoGain=Math.min(35,8+(group.items.length-1)*4+(rep.score<90?6:0));
    return {gaps,ideas,projectedLength,projectedScore,seoGain};
  }
  ...
  function buildGroups(articles){
    ...
    return [...buckets.values()].filter(x=>x.length>1).map((items,index)=>{
      ...
      const group={id:`DUP-${String(index+1).padStart(3,'0')}`,items:list,representative,intent:representative.title};
      group.plan=qualityPlan(group);
      return group;
  ...
  function renderList(){
    $('#duplicateGroupList').innerHTML=state.groups.length?state.groups.map(g=>`<button type="button" data-group-id="${g.id}" class="duplicate-list-item ${reviewed(g)?'reviewed':''}"><strong>${g.id}</strong><span>${esc(g.intent)}</span><small>${g.items.length}개 · ${reviewed(g)?'검토 완료':'검토 필요'} · 목표 ${g.plan.projectedScore}점</small></button>`).join(''):'<p class="content-empty">중복 그룹이 없습니다.</p>';
  ...
  async function queueAction(g,i,action,button){
    ...
    try{
      const response=await fetch('/api/admin/content-action',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,article:{path:i.path,title:i.title,score:i.score,duplicateGroup:g.id},representative:{path:g.representative.path,title:g.representative.title,score:g.representative.score},qualityPlan:g.plan,requestedAt:new Date().toISOString()})});
  ...
  function showGroup(id){
    ...
    const plan=g.plan;
    $('#duplicateDialogTitle').textContent=`${g.id} 중복 비교`;
    $('#duplicateDialogMeta').textContent=`검색의도 대표: ${g.intent} · ${g.items.length}개 URL`;
    $('#duplicateGroupDetail').innerHTML=`
      <div class="duplicate-recommendation"><strong>대표 유지 추천</strong><a href="${esc(g.representative.path)}" target="_blank" rel="noopener">${esc(g.representative.title)}</a><p>품질 ${g.representative.score}점 · 본문 ${g.representative.length.toLocaleString()}자 기준으로 그룹 내 가장 강한 글입니다.</p></div>
      <section class="representative-quality-plan">
        <div class="quality-plan-head"><div><span>대표글 현재 점수</span><strong>${g.representative.score}</strong></div><div><span>통합 후 목표</span><strong>${plan.projectedScore}</strong></div><div><span>예상 본문</span><strong>${plan.projectedLength.toLocaleString()}자</strong></div><div><span>예상 SEO 개선</span><strong>+${plan.seoGain}%</strong></div></div>
        <div class="quality-plan-grid"><article><h3>반드시 보강</h3><ul>${plan.gaps.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></article><article><h3>흡수할 검색의도</h3>${plan.ideas.length?`<div class="intent-chips">${plan.ideas.map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:'<p>제목 기준 추가 검색의도가 없습니다. 본문 비교 검토가 필요합니다.</p>'}</article></div>
      </section>
      ...
      <div class="duplicate-impact-note"><strong>안전 실행 순서</strong><span>원본 백업 → 대표글 보강 → 통합/삭제 계획 검토 → 내부링크 점검 → 대표 URL 연결 → 사이트맵 정리 → 배포 후 URL 검증</span></div>
      <p class="duplicate-action-message">통합·삭제 버튼은 즉시 사이트를 변경하지 않고 백업과 승인 검토 작업을 먼저 등록합니다.</p>
  ...
  function start(){ensureUI();const body=$('#contentApprovalRows');if(body)new MutationObserver(()=>{clearTimeout(start.timer);start.timer=setTimeout(refresh,180);}).observe(body,{childList:true});document.addEventListener('savingio:content-audit-complete',refresh);refresh();}
