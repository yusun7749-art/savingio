(() => {
  ...
  function normalize(input={}) {
    ...
    return {
      ...
      urlHealthCheckId:String(input.urlHealthCheckId || ''),
      urlHealthState:String(input.urlHealthState || 'unknown'),
      verifiedUrl:String(input.verifiedUrl || ''),
      httpStatus:Number(input.httpStatus || 0),
      urlCheckedAt:input.urlCheckedAt || null,
      retryRecordId:String(input.retryRecordId || ''),
      retryStatus:String(input.retryStatus || ''),
      retryAttempt:Number(input.retryAttempt || 0),
      retryMaxAttempts:Number(input.retryMaxAttempts || 3),
      nextRetryAt:input.nextRetryAt || null,
      lastFailureReason:String(input.lastFailureReason || ''),
      nextTaskId:String(input.nextTaskId || ''),
      nextTaskStatus:String(input.nextTaskStatus || ''),
      nextTaskType:String(input.nextTaskType || ''),
