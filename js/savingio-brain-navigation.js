(async()=>{
'use strict';
const VERSION='20260723-master16';
...
function scoreRecord(r,q){
 ...
 if(title===query)return 100000;if(title.startsWith(query))return 90000;if(title.includes(query))return 80000;if(keys.includes(query))return 30000;if(desc.includes(query))return 10000;if(href.includes(query))return 5000;return 0;
}
...
function collectDirectory(){
 ...
 const records=[...grid.querySelectorAll('.article-card')].map((card,index)=>{const title=card.querySelector('h2')?.textContent?.trim()||'';const desc=card.querySelector('p')?.textContent?.trim()||'';const href=card.getAttribute('href')||'';const original=card.dataset.category||card.querySelector('.card-category')?.textContent||'생활정보';const category=classify(title,original);card.dataset.category=category;const badge=card.querySelector('.card-category');if(badge)badge.textContent=category;return{card,index,title,desc,href,category,keywords:card.dataset.search||''}});
 return{grid,records};
}
function initDirectory(grid,records){
 ...
 const buttons=[...document.querySelectorAll('.category-row button[data-cat]')],pager=document.querySelector('.pager');let state={query:new URLSearchParams(location.search).get('q')||'',category:'전체'};
 const paint=source=>{const ranked=rank(records,state.query,state.category),visible=new Set(ranked.map(x=>x.r.card));records.forEach(r=>r.card.hidden=!visible.has(r.card));ranked.forEach(x=>grid.appendChild(x.r.card));count.textContent=`검색 결과 ${ranked.length}개`;if(pager)pager.hidden=Boolean(state.query||state.category!=='전체');buttons.forEach(b=>b.classList.toggle('active',(b.dataset.cat||'전체')===state.category));input.value=state.query;const u=new URL(location.href);state.query?u.searchParams.set('q',state.query):u.searchParams.delete('q');history.replaceState(null,'',u);dispatchEvent(new CustomEvent('savingio-unified-search',{detail:{query:state.query,category:state.category,ranked,source}}))};
 input.addEventListener('input',()=>{state.query=input.value.trim();state.category='전체';paint('center')});buttons.forEach(b=>b.addEventListener('click',()=>{state.query='';state.category=b.dataset.cat||'전체';paint('category')}));paint('initial');return{setQuery(q){state.query=String(q||'').trim();state.category='전체';paint('left')}};
}
async function loadRecords(local){
 if(local.length)return local;let data=window.SAVINGIO_BRAIN_DATA;
...
async function initExplorer(records,controller){
 ...
 const search=q=>q?showResults(rank(source,q)):showCategories();document.body.appendChild(nav);document.documentElement.classList.add('savingio-brain-ready');showCategories();input.addEventListener('input',()=>controller?controller.setQuery(input.value):search(input.value.trim()));nav.querySelector('form').addEventListener('submit',e=>e.preventDefault());addEventListener('savingio-unified-search',e=>{const d=e.detail||{};input.value=d.query||'';d.query?showResults(d.ranked||[]):showCategories()});
...
if(/^\/articles\/(?!$|index(?:\.html)?$)/.test(location.pathname)){if(!window.SavingioContentDNAEngine)await addScript(`/js/savingio-content-dna-engine.js?v=${VERSION}`,'savingioContentDNAEngine');else window.SavingioContentDNAEngine.install();}
