...
새 작업은 아래 순서로 검사한 뒤에만 작업표에 등록한다.

1. 작업표 전체에서 같은 작업명과 핵심 키워드를 검색한다.
2. 이름은 달라도 목적·결과물·수정 파일이 같은 유사 작업을 비교한다.
3. 이미 `[x]` 완료된 작업이면 새 항목을 만들지 않고 기존 항목에 추가 수정 또는 재검증 기록을 남긴다.
4. `[~]`, `[ ]`, `[-]`, `[!]` 상태로 존재하면 새 번호를 만들지 않고 기존 항목의 상태·세부 작업·우선순위만 갱신한다.
5. 기존 작업의 일부에 해당하면 새 상위 작업을 만들지 않고 기존 항목 아래 하위 체크리스트로 추가한다.
6. 정말 새로운 범위일 때만 다음 연속 번호로 신규 등록한다.
7. 등록 직후 현재 Phase, 현재 진행 위치, 다음 재시작 위치를 함께 갱신한다.
8. 중복 여부가 불확실하면 먼저 `중복검토` 상태로 기록하고 구현을 시작하지 않는다.

### 중복 판정 기준

다음 중 하나라도 같으면 중복 또는 유사 작업으로 본다.
...
- `신규 등록` — 기존 작업과 겹치지 않는 새 작업
- `기존 항목 병합` — 같은 목적의 기존 작업에 세부 내용 추가
- `완료 항목 재검증` — 완료 작업의 오류 확인 또는 추가 검증
- `중복 제외` — 이미 동일한 작업이 존재하여 새 항목을 만들지 않음
...
새로운 요청은 반드시 `새 작업 등록 전 중복검사 LOCK`을 통과한 뒤 추가한다.

- [x] 작업표 신규 항목 중복검사 운영 규칙 추가
  - 등록 결과: 신규 등록
  - 기존 작업표의 동일·유사·완료 작업을 먼저 비교
  - 중복이면 새 항목 생성 금지
  - 기존 항목 병합 또는 재검증으로 처리
  - 커밋: 이 문서 수정 커밋
...
- 2026-07-25: 새 작업 등록 전 중복검사 LOCK 추가
