...
- 진행 중: Savingio 인생 지도형 글 연결 리모델 1차 50개 적용 완료. 전체 220개 대상은 50개 단위 5개 배치로 고정했으며, `집→아파트→장기수선충당금→누수→보험 확인` 실제 존재 URL 연결부터 시작했다.
- 다음 실행 위치: `python -m factory.article_connection_batch --project-root . --batch 2 --batch-size 50 --apply`
