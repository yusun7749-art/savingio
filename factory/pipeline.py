...
def execute(topic: str, project_root: Path, publish: bool=False, overwrite: bool=False,
    ...
    # V2.042: repair legacy Publisher IDs before any generation work.
    pre_adsense_lock = run_adsense_lock(project_root, execute_repair=True, block_on_error=True)
    if not pre_adsense_lock["pass"]:
        raise RuntimeError(f"Publisher LOCK failed before generation: {pre_adsense_lock['blockers']}")
    adsense_identity = load_identity(project_root)
    ...
    html = ensure_html_adsense_identity(html, adsense_identity)
    ...
    post_adsense_lock = run_adsense_lock(project_root, execute_repair=True, block_on_error=True)
    if not post_adsense_lock["pass"]:
        raise RuntimeError(f"Publisher LOCK failed after generation: {post_adsense_lock['blockers']}")
    changed += [
